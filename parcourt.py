...
def parcourt_largeur(liste, depart):
    ...
    while len(file) > 0:
        case = file.pop(0)
        for v in voisins(liste, case):
            if liste[v[0]][v[1]] == 0 or liste[v[0]][v[1]] > liste[case[0]][case[1]] + 1 and v not in file:
                file.append(v)
                liste[v[0]][v[1]] = liste[case[0]][case[1]] + 1


def print_liste(liste: list, **kwargs):
    d = lens(liste)
...
print_liste(liste)
parcourt_largeur(liste, (4, 4))
print_liste(liste)
parcourt_largeur(liste, (0, 0))
print_liste(liste)
